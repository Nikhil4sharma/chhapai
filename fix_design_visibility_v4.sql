-- Robust Fix for Design Team Visibility (v4 - Final)
-- Fixed: Added ::text casting for Enum types to avoid "function lower(app_role) does not exist"
...
CREATE POLICY "orders_access_policy_v6"
  ...
  EXISTS (
    ...
    AND (
      ...
      EXISTS (
        ...
        AND (
            -- FIX: Cast role to text before lowering
            LOWER(ur.role::text) = LOWER(oi.assigned_department) OR
            LOWER(ur.role::text) = LOWER(oi.current_stage) OR
            LOWER(ur.role::text) = LOWER(oi.department)
        )
      )
      OR
      EXISTS (
...
CREATE POLICY "order_items_access_policy_v6"
...
USING (
  ...
  EXISTS (
    ...
    AND (
      ...
      LOWER(ur.role::text) = LOWER(current_stage) OR
      LOWER(ur.role::text) = LOWER(department)
    )
  )
  OR
  EXISTS (
  ...
  (
    ...
    (
        ...
        OR
        -- FIX: Cast role to text before comparison (safe practice)
