-- Migration to add DYNAMIC AGENT NAME MATCHING
-- This allows auto-assignment if WC Agent Name matches Profile Full Name (e.g. "Rahul" -> "Rahul")
...
BEGIN
  ...
  -- 🕵️ AGENT MAPPING LOGIC (ROBUST + DYNAMIC)
  ...
  IF v_agent_name IS NOT NULL THEN
    ...
    -- A. Hardcoded Overrides (Priority)
  ...
  -- 🆕 5. DYNAMIC NAME MATCHING (If no email match yet)
  IF v_assigned_user_id IS NULL AND v_clean_agent_name IS NOT NULL THEN
    -- Try to match against profiles.full_name
    SELECT user_id INTO v_assigned_user_id
    FROM profiles
    WHERE 
      -- Normalize profile name same way: footer case, no spaces
      LOWER(REPLACE(REPLACE(full_name, ' ', ''), '.', '')) = v_clean_agent_name
      OR 
      -- Helper: Check if profile name starts with agent name (e.g. Agent "Rahul" matches "Rahul Kumar")
      LOWER(REPLACE(full_name, ' ', '')) LIKE v_clean_agent_name || '%'
    LIMIT 1;
    
    IF v_assigned_user_id IS NOT NULL THEN
        RAISE NOTICE '✅ Auto-Assign: Matched Agent Name % to User ID % via Name Lookup', v_agent_name, v_assigned_user_id;
    END IF;
  END IF;
  
  -- 6. Fallback to payload assigned_user_id (Only if UUID)
