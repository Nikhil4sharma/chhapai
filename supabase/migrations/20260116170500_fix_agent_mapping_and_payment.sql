-- Migration to fix Agent Mapping and Payment Status in import_wc_order
...
BEGIN
  ...
  IF v_agent_name IS NOT NULL THEN
    IF LOWER(v_agent_name) = 'nikhilsharma' THEN v_assigned_email := 'chd+1@chhapai.in';
    ELSIF LOWER(v_agent_name) = 'rohiniraina' THEN v_assigned_email := 'rohini@chhapai.in';
    ELSIF LOWER(v_agent_name) = 'work' THEN v_assigned_email := 'work@chhapai.in';
    END IF;
  END IF;
  ...
  -- 5. Fallback to payload assigned_user_id
  ...
  -- Default to 'pending' to match manual import behavior (User Request)
  -- Only set to 'paid' if explicitly marked as paid in payload specifically
