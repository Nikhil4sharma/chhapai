-- Enforce Customer Assignment Sync with Order
-- Ensures that when an order is imported, the customer is assigned to the SAME agent as the order.
...
BEGIN
  ...
  -- If we STILL don't have a user, assign it to a random/first 'sales' role user
  ...
  -- 1️⃣ CUSTOMER UPSERT
  -- CRITICAL UPDATE: ALWAYS update assigned_to if v_assigned_user_id is found
