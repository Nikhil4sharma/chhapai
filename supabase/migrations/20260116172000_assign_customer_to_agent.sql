-- Migration to ASSIGN CUSTOMER TO AGENT in import_wc_order
...
BEGIN
  ...
  -- 🕵️ AGENT MAPPING LOGIC (Keep Robust Logic)
  ...
  IF v_agent_name IS NOT NULL THEN
    ...
    IF v_clean_agent_name IN ('nikhilsharma', 'nikhil') THEN v_assigned_email := 'chd+1@chhapai.in';
    ELSIF v_clean_agent_name IN ('rohiniraina', 'rohini') THEN v_assigned_email := 'rohini@chhapai.in';
    ELSIF v_clean_agent_name IN ('work', 'workagent', 'office') THEN v_assigned_email := 'work@chhapai.in';
    END IF;
  END IF;
  ...
  -- FORCE 'pending' to match manual import behavior.
  ...
  -- 1️⃣ CUSTOMER UPSERT (with assigned_to)
  ...
  VALUES (
    ...
    v_assigned_user_id -- Determine from logic above
  )
  ...
  DO UPDATE SET
    ...
    -- If current order has no agent, don't unassign the customer.
