-- Robust Fix for Design Team Visibility
-- Checks BOTH public.user_roles and public.profiles for department/role info
...
CREATE POLICY "orders_access_policy_v4"
...
USING (
  ...
  OR EXISTS (
    ...
    WHERE user_id = auth.uid()
    AND (
      role IN ('admin', 'sales', 'super_admin', 'accounts') OR
    ...
    AND (
      ...
      EXISTS (
        ...
        AND (
            ...
            LOWER(p.department) = LOWER(oi.department) OR
            LOWER(p.role) = LOWER(oi.assigned_department)
        )
      )
    )
  )
...
CREATE POLICY "order_items_access_policy_v4"
...
USING (
  ...
  EXISTS (
    ...
    AND (
      ...
      LOWER(p.department) = LOWER(department) OR
       LOWER(p.role) = LOWER(assigned_department)
    )
  )
  OR
  ...
  (
    ...
    AND 
    (
        EXISTS (SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND (department = 'design' OR role = 'design'))
        OR
