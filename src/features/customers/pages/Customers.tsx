...
export default function Customers() {
    const { customers, isLoading, refetch, deleteCustomer, deleteAllCustomers } = useWooCommerce();
    const [searchTerm, setSearchTerm] = useState('');
    const [filter, setFilter] = useState<'all' | 'high-value' | 'repeat'>('all');
    ...
    const processedCustomers = useMemo(() => {
        let result = customers.filter(c => {
            ...
            const address = JSON.stringify(c.billing).toLowerCase(); // broad search in address
    ...
    const handleDownloadCSV = () => {
        const headers = ["ID", "Name", "Email", "Phone", "Total Spent", "Orders", "City"];
